...
export default class TableRow extends Component {
  ...
  _renderLeftButtons = (buttons, maxWidth) => {
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      let buttonControl = (
        <TouchableNativeFeedback key={`left-${i}`} onPress={onAction(isRemove, onPress)} background={TouchableNativeFeedback.SelectableBackground()}>
  ...
  _renderRightButtons = (buttons, maxWidth) => {
    ...
    for (var i = buttons.length - 1; i >= 0; i--) {
      ...
      let buttonControl = (
        <TouchableNativeFeedback key={`right-${i}`} onPress={onAction(isRemove, onPress)} background={TouchableNativeFeedback.SelectableBackground()}>
          <Animated.View style={buttonStyles}>
            <Animated.View style={contentStyles}>
              { contents }
            </Animated.View>
