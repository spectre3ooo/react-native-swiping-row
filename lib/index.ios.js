...
export default class TableRow extends Component {
  ...
  _renderLeftButtons = (buttons, maxWidth) => {
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      let buttonControl = (
        <TouchableHighlight key={`left-${i}`} onPress={() => this.onAction(isRemove, onPress)} onLongPress={onLongPress}>
  ...
  _renderRightButtons = (buttons, maxWidth) => {
    ...
    for (var i = buttons.length - 1; i >= 0; i--) {
      ...
      let buttonControl = (
        <TouchableHighlight key={`right-${i}`} onPress={() => this.onAction(isRemove, onPress)} onLongPress={onLongPress}>
          <Animated.View style={buttonStyles}>
						<Animated.View style={contentStyles}>
							{contents}
						</Animated.View>
